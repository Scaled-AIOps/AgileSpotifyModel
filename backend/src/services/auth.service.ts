/**
 * Purpose: Authentication service — login, refresh, register, signet change.
...
export async function register(email: string, signet: string, name: string, role: Role): Promise<{ accessToken: string; refreshToken: string; user: Omit<User, 'signetHash'> }> {
  const existingId = await redis.get(`user:email:${email}`);
  if (existingId) throw createError('Email already registered', 409);

  const id = generateId();
  const memberId = generateId();
  const signetHash = await hashSignet(signet);
  const now = new Date().toISOString();

  const pipeline = redis.pipeline();
  pipeline.hset(`user:${id}`, { id, email, signetHash, role, memberId, createdAt: now });
...
export async function login(email: string, signet: string): Promise<{ accessToken: string; refreshToken: string; user: Omit<User, 'signetHash'> }> {
  const userId = await redis.get(`user:email:${email}`);
  if (!userId) throw createError('Invalid credentials', 401);

  const data = await redis.hgetall(`user:${userId}`);
  if (!data?.signetHash) throw createError('Invalid credentials', 401);

  const valid = await compareSignet(signet, data.signetHash);
...
export async function getMe(userId: string): Promise<Omit<User, 'signetHash'> | null> {
  const data = await redis.hgetall(`user:${userId}`);
  if (!data?.id) return null;
  return { id: data.id, email: data.email, role: data.role as Role, memberId: data.memberId, createdAt: data.createdAt };
}

export async function loginByEmail(email: string): Promise<{ accessToken: string; refreshToken: string; user: Omit<User, 'signetHash'> }> {
...
export async function changeSignet(userId: string, currentSignet: string, newSignet: string): Promise<void> {
  const data = await redis.hgetall(`user:${userId}`);
  if (!data?.signetHash) throw createError('User not found', 404);

  const valid = await compareSignet(currentSignet, data.signetHash);
  if (!valid) throw createError('Current signet is incorrect', 400);

  const newHash = await hashSignet(newSignet);
  await redis.hset(`user:${userId}`, 'signetHash', newHash);

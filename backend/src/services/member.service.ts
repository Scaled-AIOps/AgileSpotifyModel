...
export async function create(data: {
  name: string; email: string; passcode?: string; role: Role; avatarUrl: string; squadId: string;
}): Promise<Member> {
  ...
  if (data.passcode) {
    ...
    pipeline.hset(`user:${userId}`, { id: userId, email: data.email, passcodeHash, role: data.role, memberId: id, createdAt: now });
